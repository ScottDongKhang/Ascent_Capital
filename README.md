...
**Task 0B — Disagreement score** (`debate/disagreement_scorer.py`): measures semantic disagreement between agent reasoning traces using TF-IDF cosine similarity (pure numpy, no external API). Formula: `1 - mean([sim(bull,bear), sim(bull,devil), sim(bear,devil)])`. Score written to every verdict JSON. Judge receives a formatted fragment — if score > 0.70 (soft consensus), judge is instructed to bias toward `reduce_size` or `halt_and_review`. Also serves as the validation metric for private context subsets (Task 0.2): if `mean_disagreement_score` does not drop after `_build_agent_context()` goes live, the section builder design needs revisiting. Source: Tajik et al. (2026), arXiv:2601.12618.
