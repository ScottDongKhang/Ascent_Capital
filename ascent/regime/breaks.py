"""
ascent/regime/breaks.py
------------------------
Structural break / change-point detection module.

Purpose: DIAGNOSTIC ONLY.
  • Identifies major structural breaks in market-state feature series.
  • Used to:
      1. Flag dates where the latent regime model may be stale.
      2. Recommend re-fit triggers.
      3. Diagnose instability in transition behavior.
  • This module NEVER drives live regime decisions directly.
    Its outputs are annotations on the monitoring dashboard.

Uses the `ruptures` library (Pelt or Binseg algorithm).
Gracefully skips if ruptures is unavailable.
"""
...
try:
    ...
    _HAS_RUPTURES = False
    log.warning("regime.breaks: ruptures not installed — break detection disabled")
...
def _penalty_from_series(s: np.ndarray, multiplier: float = 2.0) -> float:
    """
    Heuristic BIC-like penalty.
    multiplier * log(n) * sigma^2 is a common choice.
    """
    n = len(s)
...
class BreakDetector:
    ...
    ----------
    min_size : int
        Minimum number of samples between breakpoints (ruptures parameter).
    penalty_multiplier : float
        Scale factor for the automatic penalty calculation.
    algorithm : str
        'pelt' (exact, preferred) or 'binseg' (approximate, faster for long series).
    """
    ...
    def detect(
        ...
        target_columns : list of str, optional
            Which columns to analyze. If None, uses all columns.

        Returns
        -------
        dict mapping feature_name -> BreakResult
        """
        if not _HAS_RUPTURES:
            log.warning("regime.breaks: ruptures unavailable — returning empty results")
            return {}
    ...
    def latest_zscore(
        ...
        """
        Return a z-score representing how anomalous the most recent observations
        are relative to the historical baseline.

        Algorithm:
          For each feature column, compute the mean absolute z-score of the
          recent window (last `lookback_recent` rows) relative to the baseline
          distribution (preceding `lookback_baseline` rows). Return the average
          across all features.

          This is a continuous anomaly signal — it spikes when recent values
          diverge from the historical baseline in any direction.

        Returns 0.0 if ruptures is unavailable, feature_panel is None, or
        insufficient history.

        Parameters
        ----------
        feature_panel : pd.DataFrame
            Full feature panel with DatetimeIndex.
        lookback_recent : int
            Days of recent data to average for the "current" reading (default 5).
        lookback_baseline : int
            Days of preceding history for the baseline mean/std (default 63).
        """
        if not _HAS_RUPTURES or feature_panel is None:
            return 0.0
    ...
    def should_refit(
        ...
        This is a diagnostic recommendation only — never drives a hard switch.
        """
        if not _HAS_RUPTURES:
            return False
