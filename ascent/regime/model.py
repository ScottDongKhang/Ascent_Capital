"""
ascent/regime/model.py  [patched: multi-state stability]
K==2  -> MarkovRegression primary, HMM fallback
K>=3  -> GaussianHMM primary (10 restarts), MarkovRegression fallback
Changes: DEGENERATE_STATE_THRESHOLD 0.05->0.03, search_reps 5->15,
         HMM diag covariance + multi-restart, K>=3 uses HMM in selection,
         label_states K==3 uses EUPHORIC, rich diagnostics logged.
"""
...
class _HMMModel:
    ...
    def __init__(self, k_regimes: int, n_iter: int = 300, n_restarts: int = 10):
        ...
        self._fitted = False
        self._train_len = 0
        # FIX #3: store the actual fitted training matrix so aic/loglike
    ...
    def _impute(self, x: np.ndarray, fit: bool = False) -> np.ndarray:
        # FIX #3: only fit imputer medians on training data (fit=True).
    ...
    def fit(self, features: np.ndarray) -> bool:
        ...
        log.info(f"regime.model[HMM] K={self.k_regimes}: transmat=\n{np.round(best_model.transmat_,3)}")
        n_degen = int(np.sum(fracs < DEGENERATE_STATE_THRESHOLD))
        print("HMM K=", self.k_regimes, "fracs=", fracs, "n_degen=", n_degen)
        ...
        self._train_matrix = x          # FIX #3: store real training matrix
        self._fitted = True
        return True
    ...
    def filtered_probs(self, features: np.ndarray) -> np.ndarray:
        ...
        x = self._impute(x, fit=False)  # FIX #2/#3: use train-fitted imputer
        try:
    ...
    @property
    def aic(self):
        # FIX #3: score on the real training matrix, not a zero array
...
class _PCATransformer:
    """
    FIX #2: fit PCA and imputer on training data only, then reuse on test data.
    Before this fix, _feature_to_signal() called fit_transform() independently
    on both train and test, meaning PCA components were computed separately for
    each split. This is leakage: the test PCA saw test-period variance.
    """
    ...
    def transform(self, features: pd.DataFrame) -> np.ndarray:
        ...
        x = features.values.astype(float)
        # Use train-fitted imputer medians
        inds = np.where(np.isnan(x))
        if inds[0].size > 0:
            x = x.copy()
            x[inds] = np.take(self._imputer_medians, inds[1])
        # Align columns if test has fewer features than train (edge case)
...
def _evaluate_candidate(k, train_feat, test_feat, test_ret):
    """
    FIX #2: fit PCA/imputer on train only and reuse transformer on test.
    Before this fix both _feature_to_signal(train_feat) and
    _feature_to_signal(test_feat) called fit_transform() independently,
    so the test PCA was computed on test-period data — leakage.
    """
    try:
        if k == 2 and _HAS_STATSMODELS:
            # FIX #2: use _PCATransformer to fit on train and transform test
            ...
            if not hmm.fit(train_feat.values):
                return None
            # FIX #2: use hmm's own train-fitted scaler/imputer to transform test
...
def walk_forward_model_select(feature_panel, spy_returns, candidate_k=(2,3,4),
                               train_days=756, test_days=126, step_days=63, min_train=504):
...
def _select_best_k(scorecards, candidate_k):
    ...
    for k in candidate_k:
        ...
        if records:
            valid[k] = len(records)
    print('REGIME SELECTION valid candidates:', valid)
    if not valid:
        print('REGIME SELECTION: no valid candidates, defaulting K=2')
        return 2
    best, best_score = None, -np.inf
    for k in valid:
        sc = _make_scorecard(scorecards[k], k, 252)
        print(f'REGIME SELECTION K={k} composite={sc.composite_score:.4f}')
        if sc.composite_score > best_score:
            best_score, best = sc.composite_score, k
    print('REGIME SELECTION: chose K=', best, 'score=', round(best_score,4))
    return best
...
class RegimeModel:
    def __init__(self, k_regimes: int = 3):
        ...
        self._fitted = False
        # FIX #2: store PCA transformer fitted on training data for reuse
    ...
    def _fit_markov_primary(self, feature_panel):
        try:
            # FIX #2: fit PCA transformer on training data and store for later use
    ...
    def _fit_hmm_primary(self, feature_panel):
        try:
            hmm = _HMMModel(k_regimes=self.k_regimes, n_restarts=10)
    ...
    def predict_probs(self, feature_panel: pd.DataFrame) -> pd.DataFrame:
        ...
        if self._active_backend == "markov":
            # FIX #2: use stored PCA transformer to avoid re-fitting on prediction data
