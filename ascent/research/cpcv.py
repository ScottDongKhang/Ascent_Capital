...
class CPCVSplitter:
    ...
    ----------
    n_splits : int
        N — number of folds. Default 6.
    n_test_splits : int
        k — test folds per combination. Default 2.
    purge_days : int
        Business days to remove from train immediately *before* each test fold.
        Set to max feature lookback (mom_5d → 5 days).
    embargo_days : int
        Business days to remove from train immediately *after* each test fold.
        Set to 5 days.
    """
    ...
    def split(
        ...
        dates : pd.DatetimeIndex
            Full ordered date index to split.

        Yields
        ------
        (train_dates, test_dates) : tuple of DatetimeIndex
        """
