...
| `ascent/risk` | Factor model, covariance, exposure bounds, correlation guard, PM validator. | `compute_factor_loadings`, `compute_portfolio_exposures`, `check_factor_bounds`, `check_cross_agent_correlation`, `apply_correlation_adjustments`, `validate` (pm_risk_validator), `classify_regime` |
...
| Change debate behaviour | `debate/agents.py`, `debate/judge.py`, `debate/adversarial_authority.py` | `_AGENT_SECTIONS`, `REGIME_PLAYBOOK`, `run_judge`, `get_authority`. Debate is advisory-only — no write path to portfolio or execution. Verdicts produce advisory position change suggestions for logging and offline analysis. |
...
| Change AI PM authority / guardrails | `ascent/strategy/earned_authority.py`, `ascent/strategy/ai_pm_guardrails.py` | `LEVEL_WEIGHTS`, `update_authority`, `blend`. `apply_guardrails` and `_LEVEL_CONFIG` were deleted 2026-08-16 (zero non-test callers). |
