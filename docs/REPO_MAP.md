...
| `ascent/alpha` | One sleeve per module + the blending stack and IC gate. | `build_alpha_stack`, `alpha_to_ranks`, `DEFAULT_ALPHA_WEIGHTS`, `DEFAULT_ALPHA_WEIGHTS_BY_REGIME`, `IC_GATE_THRESHOLD`, `trend_alpha`, `meanrev_alpha`, `build_ml_alpha`, `_SPARSE_FILL_ZERO`, `SleeveMetaLearner` |
| `ascent/portfolio` | Weight construction, caps, overlays, optimizers. | `enforce_constraints`, `_water_fill_cap`, `enforce_cluster_cap`, `enforce_risk_budget_cap`, `sector_constrained_weighted_mvo`, `SectorDataError`, `apply_exposure_overlays`, `ma_filter_scale`, `vol_target_scale`, `apply_hedge_overlay`, `optimize_mvo`, `black_litterman_views`, `tc_aware_weights`, `build_long_short_weights` |
...
| 2264 / 2307 | `_apply_position_change_to_weights` / `apply_judge_position_change` (the one sanctioned debate write path) |
