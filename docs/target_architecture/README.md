...
| `23_empirical_alpha_audit.md` | Real per-sleeve IC computed from three recovered backup copies of a deleted log file: the live sleeves (meanrev, statarb) measure as the *weakest* of the sleeves with any data, dormant trend/insider initially appeared to measure 2-7x better, and meanrev would already be mechanically zeroed by the system's own gate at the last measured point. Also corrects the "gate is broken" framing — it's a deliberate deletion plus paused trading, not a bug. **The trend/insider ranking is corrected in `27_trend_insider_reconciliation.md` — see below, trend specifically was reversed.** |
| `27_trend_insider_reconciliation.md` | **Corrects `23`'s trend/insider finding.** The formal, larger-sample proof audit (the actual governance gate) found `trend` has a statistically significant *negative* IC — an anti-signal, correctly cut — while the recovered-log slice that looked good was an expanding-window statistical illusion (21 "observations" that were really ~1 effective sample). **Trend should be dropped, not pursued.** `insider` remains a genuine open question (positive but not significant) — collect more data and rerun the formal audit, don't promote on existing evidence. |
