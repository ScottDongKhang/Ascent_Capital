...
# ── Task 6: Monthly report ────────────────────────────────────────────────────

def test_monthly_report_generated_without_error(tmp_path):
    """Mock data → PDF file created without error."""
    from ascent.reporting import investor_report as ir

    with patch.object(ir, "_pnl_for_month", return_value=[
        {"date": "2026-05-01", "nav": 100_000, "daily_return": 0.01, "spy_return": 0.008},
        {"date": "2026-05-02", "nav": 101_000, "daily_return": 0.01, "spy_return": 0.007},
    ]):
        with patch.object(ir, "_slippage_for_month", return_value=[]):
            with patch.object(ir, "_generate_commentary", return_value="Test commentary paragraph one.\n\nParagraph two."):
                with patch.object(ir, "_risk_disclosures", return_value="Risk disclosure text."):
                    out = ir.generate_monthly_report(2026, 5, output_path=tmp_path / "test_report.pdf")
...
def test_report_contains_required_sections(tmp_path):
    """Generated PDF text contains required section headers."""
    from ascent.reporting import investor_report as ir
    from reportlab.pdfgen import canvas

    # Generate a real (small) report
    with patch.object(ir, "_pnl_for_month", return_value=[
        {"date": "2026-05-01", "nav": 100_000, "daily_return": 0.005, "spy_return": 0.003},
    ]):
        with patch.object(ir, "_slippage_for_month", return_value=[]):
            with patch.object(ir, "_generate_commentary", return_value="Commentary here."):
                with patch.object(ir, "_risk_disclosures", return_value="RISK DISCLOSURES\n\nPast performance is not indicative of future results."):
                    out = ir.generate_monthly_report(2026, 5, output_path=tmp_path / "sections_test.pdf")

    # Verify PDF was created and is non-trivial
    assert out.exists()
